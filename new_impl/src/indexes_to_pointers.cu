...
void procedures::cuda::indexes_to_pointers(int INPUT_ARRAY idx, int idx_len, int ** inter, int * ptr, int ptr_len) {

    *inter = utils::cuda::allocate_zero<int>((HISTOGRAM_BLOCKS+1) * ptr_len);

    parallel_histogram2_kernel<<<HISTOGRAM_BLOCKS, 1024>>>(idx, idx_len, *inter, ptr_len);
    CUDA_CHECK_ERROR
    ...
    *_inter = inter;
    ...
    for(int tid = 0; tid < HISTOGRAM_BLOCKS; tid++) {
        ...
        for(int i = 0; i < BLOCK_SIZE && START_INTRA + i < idx_len; i++) {
            ...
            if(intra != NULL) { intra[START_INTRA + i] = inter[index]; }
            inter[index]++;
        }
    }
